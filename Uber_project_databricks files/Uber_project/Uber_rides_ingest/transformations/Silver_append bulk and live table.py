...
#get schema from the bulk_rides table
...
#create an empty stage streaming table
dp.create_streaming_table("stg_rides")

#appending bulk/initial load to empty streaming table
@dp.append_flow(
    target= "stg_rides"
)
def append_bulk_rides():
        df = spark.readStream.table("uber.bronze.bulk_rides")
        df = df.withColumn("booking_timestamp", col("booking_timestamp").cast("timestamp"))
        return df

#appending streaming table after initial load.
@dp.append_flow(
    target= "stg_rides"
)
def Stream_rides():
        df = spark.readStream.table("uber.bronze.rides_raw")
        #transfrom the rides_raw before appending
        df_parced = df.withColumn("parced_rides", from_json(col("rides"),rides_schema))\
                    .select("parced_rides.*")
        return df_parced
